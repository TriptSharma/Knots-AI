...
class Database:
    ...
    def get(self, token=None, get_dict=False, json_kwargs={}):
        ...
        if build_query.get("orderBy"):
            if build_query["orderBy"] == "$key":
                if get_dict==True:
                    return sorted(request_dict.items(), key=lambda item: item[0])
                sorted_response = sorted(request_dict.items(), key=lambda item: item[0])
            elif build_query["orderBy"] == "$value":
                if get_dict==True:
                    return sorted(request_dict.items(), key=lambda item: item[1])
                
                sorted_response = sorted(request_dict.items(), key=lambda item: item[1])
            else:
                if get_dict==True:
                    return sorted(request_dict.items(), key=lambda item: item[1][build_query["orderBy"]])
